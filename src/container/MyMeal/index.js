...
import styles from '../MyMeal/style';
...
const MyMeal = props => {
  ...
  const onS = resolve => {
    ...
    setCombinedFood(data.map(obj => ({...obj, isSelected: false})));
    console.log(
      'result====',
      data.map(obj => ({...obj, isSelected: false})),
    );
  };
  const onF = reject => {
    setTimeout(() => {
          ...
          <View style={[globalStyles.center, globalStyles.padding_40]}>
            ...
            {selectedFoods.length > 0 && (
              <TouchableOpacity
                ...
                <Text style={globalStyles.btn_heading_black}>ADD TO TODAY</Text>
              </TouchableOpacity>
            )}
            {/* <Text
              onPress={() => navigation.navigate('EditFavoriteFoods')}
