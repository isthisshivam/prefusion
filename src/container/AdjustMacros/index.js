...
import styles from '../AdjustMacros/style';
...
const AdjustMacros = () => {
  ...
  const getCalorieChangeValue = type => {
    ...
    console.log('getCalorieChangeValue', JSON.stringify(payload));
    dispatch(changeCalorieCounterRequest(payload, onSSS, onFFF));
  };
  const onSSS = resolve => {
    console.log('onSSS.resolve', resolve.data);
    setSuggestedCalories(resolve.data.calories);
  };
  const onFFF = reject => {
    console.log('reject', reject);
    if (reject == 'Please update Weight, height and age') {
      setTimeout(() => {
        Utility.getInstance().inflateToast(reject);
      }, 100);
      setTimeout(() => {
  ...
  const onCustomCalriesChange = value => {
    setcustomCalories(parseInt(value));
  };
              ...
              <View
                ...
                {isLoggedIn && (
                  <View>
                    ...
                    <SelectDropdown
                      data={goalTypeArray}
                      onSelect={(selectedItem, idex) => {
                        console.log('selectedItem==', selectedItem);
                        setGoalType(selectedItem);
                        setGoalTypeId(selectedItem.id);
                        getCalorieChangeValue(selectedItem);
                      }}
