...
const MostRecentMealView = props => {
  const {data, onClick, onEditPress} = props;

  const RenderMeals = props => {
    const {index} = props.item;
    const {item, onClick} = props;
    console.log('rendermeals==', item?.item?.image);
    return (
      <View
        style={[
  ...
  return (
    <FlatList
      extraData={data}
      data={data}
      ListEmptyComponent={<ListEmptyComponent heading></ListEmptyComponent>}
...
export default MostRecentMealView;
